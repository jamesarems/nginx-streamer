...
RUN pushd nginx-1.19.1 ; ./configure --add-module=../nginx-rtmp-module --add-module=../nginx-vod-module --add-module=../ngx_cache_purge --add-module=../headers-more-nginx-module --with-file-aio --with-http_ssl_module --with-cc-opt="-Wimplicit-fallthrough=0" ; make ; make install ; popd
...
RUN rm -rf nginx-1.19.1.tar.gz nginx-1.19.1 nginx-rtmp-module nginx-vod-module headers-more-nginx-module ngx_cache_purge
RUN dnf clean all
